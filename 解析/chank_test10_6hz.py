        ...
        if ser.in_waiting > 0:  # 受信データがあるか確認
            ...
            if result:
                ...
                if measurement_command.value == 2: # 10Hz計測開始命令
                    ...
                    minima_indices_topn, minima_values_topn = find_local_minima(np.array(store_list_look_10hz), top_n=50) #極小値top50を求める.
                    with lock:
                    ...
                    maxima_indices_topn, maxima_values_topn = find_local_maxima(np.array(store_list_look_6hz), top_n=50) #極大値top50を求める.
                    minima_indices_topn, minima_values_topn = find_local_minima(np.array(store_list_look_6hz), top_n=50) #極小値top50を求める.
                    with lock:
                    ...
                    maxima_indices_topn, maxima_values_topn = find_local_maxima(np.array(store_list_non_look_10hz), top_n=50) #極大値top50を求める.
                    minima_indices_topn, minima_values_topn = find_local_minima(np.array(store_list_non_look_10hz), top_n=50) #極小値top50を求める.
                    with lock:
                        threshold_non_look_10hz_max.value = np.mean(maxima_values_topn)
                        threshold_non_look_10hz_min.value = np.mean(minima_values_topn)

                    maxima_indices_topn, maxima_values_topn = find_local_maxima(np.array(store_list_non_look_6hz), top_n=50) #極大値top50を求める.
                    minima_indices_topn, minima_values_topn = find_local_minima(np.array(store_list_non_look_6hz), top_n=50) #極小値top50を求める.
                    with lock:
...
# 1000Hzでデータ要求を送信しないで、受信も行い、データの数をカウントする関数
def communicate_and_count_test(ser , received_list_1, receive_value_1, received_list_2, receive_value_2, received_list_3, receive_value_3, received_list_4, receive_value_4, clock_signal_1, clock_signal_2, lock):
    start_time = time.perf_counter()  # 計測開始時間
    data_count = 0  # データのカウント
    t = 1
...
def func_visual(priority, flag_blink_1, flag_blink_2, lock, chank_list_1, adjust_chank_list_1, chank_list_2, adjust_chank_list_2, chank_list_3, adjust_chank_list_3, chank_list_4, adjust_chank_list_4, gaze_flag_1, gaze_flag_1_2, gaze_flag_2, gaze_flag_2_2):
    p = psutil.Process()
    p.nice(priority)  # psutilで優先順位を設定
    print(f"Process (func_visual) started with priority {priority}")
...
def func_serial(priority, com, shared_receive_list_1, receive_value_1, shared_receive_list_2, receive_value_2,shared_receive_list_3, receive_value_3,shared_receive_list_4, receive_value_4, clock_signal_1, clock_signal_2, lock):
    p = psutil.Process()
    p.nice(priority)  # psutilで優先順位を設定
    ...
    communicate_and_count_test(ser, shared_receive_list_1, receive_value_1, shared_receive_list_2, receive_value_2,shared_receive_list_3, receive_value_3,shared_receive_list_4, receive_value_4, clock_signal_1, clock_signal_2, lock)
...
def func_chank(priority, receive_value, receive_value_2,  flag_blink, chank_list, chank_list_2, clock_signal, adjust_chank_list, adjust_chank_list_2, analysis_flag, chank_size, lock):
    """
    ...
    chank_chank_list_1_2 = [] #buffer1_2

    chank_chank_list_2 = [] #buffer2
    chank_chank_list_2_2 = [] #buffer2_2
            ...
            if flag_blink.value == True:
                if len(chank_chank_list_2) != 0:
                    ...
                    with lock:
                        ...
                        chank_list_2.append(chank_chank_list_2_2)

                        chank_list_copy = copy.deepcopy(list(chank_chank_list_2))
                        chank_list_copy_2 = copy.deepcopy(list(chank_chank_list_2_2))

                        adjust_chank_list.append(adjust_data_to_size(chank_list_copy, target_size=chank_size))
                        adjust_chank_list_2.append(adjust_data_to_size(chank_list_copy_2, target_size=chank_size))
                ...
                with lock:
                    if isinstance(receive_value, ListProxy) and len(receive_value) > 0 and clock_signal.value == True:
                        ...
                        chank_chank_list_1_2.append(receive_value_2[0])
                ...
                if len(chank_chank_list_1) != 0:
                    ...
                    with lock:
                        ...
                        chank_list_2.append(chank_chank_list_1_2)

                        chank_list_copy = copy.deepcopy(list(chank_chank_list_1))
                        chank_list_copy_2 = copy.deepcopy(list(chank_chank_list_1_2))
                ...
                with lock:
                    if isinstance(receive_value, ListProxy) and len(receive_value) > 0 and clock_signal.value == True:
                        ...
                        chank_chank_list_2_2.append(receive_value_2[0])
...
def func_analysis2(priority, adjust_chank_list_1, adjust_chank_list_1_2, analysis_flag_1, gaze_flag_1, gaze_flag_1_2, gaze_flag_2, gaze_flag_2_2, adjust_chank_list_2, adjust_chank_list_2_2, analysis_flag_2, lock, threshold_look_10hz_max, threshold_look_6hz_max, threshold_non_look_10hz_max, threshold_non_look_6hz_max, threshold_look_10hz_min, threshold_look_6hz_min, threshold_non_look_10hz_min, threshold_non_look_6hz_min):
    p = psutil.Process()
    p.nice(priority)  # psutilで優先順位を設定
    print(f"Process (func_analysis) started with priority {priority}")
    chank_copy = [] #10Hz用.
    chank_copy_2 = [] #高Q-10Hz用.

    chank_copy2 = [] #6Hz用.
    chank_copy2_2 = [] #高Q-6Hz用.
    # flag = False
    count = 0
    count2 = 0
    time.sleep(3)
    ...
    while True:
        if analysis_flag_1.value == True:
            ...
            # plot_multiple_lines(chank_copy,chank_copy_2, count, gaze_flag_1, gaze_flag_1_2, "10Hz", 0, 0.1, 100)
            # plot_phase_ana(chank_copy,chank_copy_2, count, gaze_flag_1, gaze_flag_1_2, "10Hz", 1, 20, 20, 100)
            plot_multiple_lines_2(chank_copy,chank_copy_2, count, gaze_flag_1, gaze_flag_1_2, "10Hz", 0, 0.1, 100)
            phase_ana(chank_copy,chank_copy_2, count, gaze_flag_1, gaze_flag_1_2, "10Hz", 1, 20, 20, 100,threshold_look_10hz_max, threshold_look_10hz_min, threshold_non_look_10hz_max, threshold_non_look_10hz_min) # fre_change_word.
            count = count + 1
        ...
        if analysis_flag_2.value == True:
            with lock:
                ...
                chank_copy2_2 = adjust_chank_list_2_2[-20:] #最後の20個のデータをコピー

                analysis_flag_2.value = False
            # plot_multiple_lines(chank_copy2,chank_copy2_2, count2, gaze_flag_2, gaze_flag_2_2, "6Hz", 0, 0.167, 167) # fre_change_word.
            # plot_phase_ana(chank_copy2,chank_copy2_2, count2, gaze_flag_2, gaze_flag_2_2, "6Hz", 1, 20, 20, 167)    # fre_change_word.
            plot_multiple_lines_2(chank_copy2,chank_copy2_2, count2, gaze_flag_2, gaze_flag_2_2, "6Hz", 0, 0.167, 167) # fre_change_word.
            phase_ana(chank_copy2,chank_copy2_2, count2, gaze_flag_2, gaze_flag_2_2, "6Hz", 1, 20, 20, 167, threshold_look_6hz_max, threshold_look_6hz_min, threshold_non_look_6hz_max, threshold_non_look_6hz_min)
            count2 = count2 + 1




def plot_multiple_lines(y_values,y_values_2, count, gaze_flag, gaze_flag2, folder, start, end, num_points): #平均値の追加
    """
    start, # 開始値
    end, # 終了値
    num_points # 生成する数値の数
...
def plot_phase_ana(y_values,y_values_2, count, gaze_flag, gaze_flag2, folder, start, end, num_points, range_ms): #位相分析
    x = np.linspace(start, end, num_points)  # 0から10までの100個の等間隔の点
...
def plot_multiple_lines_2(y_values,y_values_2, count, gaze_flag, gaze_flag2, folder, start, end, num_points): #平均値の追加
    """
    start, # 開始値
    end, # 終了値
    num_points # 生成する数値の数
    ...
    if count % 20 == 0:
        ...
        plt.legend(loc='upper right')
        plt.grid(True)
        file_name_path = f'./plt_img/add_ave_high_q/{folder}/{current_time}.png'
        plt.savefig(file_name_path, dpi=70)
        plt.close()
...
def phase_ana(y_values,y_values_2, count, gaze_flag, gaze_flag2, folder, start, end, num_points, range_ms, threshold_look_max, threshold_look_min, threshold_non_look_max, threshold_non_look_min): #位相分析
    """
    y_values : 10Hz通常データ.
    y_values_2 : 10Hz高Qデータ.
    """
    x = np.linspace(start, end, num_points)  # 0から10までの100個の等間隔の点


    max_indices_per_row = np.argmax(y_values, axis=1) # 各行の最大値のインデックスを取得. 要素数は20個
    min_indices_per_row = np.argmin(y_values, axis=1) # 各行の最小値のインデックスを取得. 要素数は20個
    max_values_per_row = np.max(y_values_2, axis=1) # 各行の最大値を取得. 要素数は20個
    min_values_per_row = np.min(y_values_2, axis=1) # 各行の最小値を取得. 要素数は20個
    ...
    if range_ms == 100: #10Hzの場合
        ...
        if len(max_values_per_row[(max_values_per_row >= threshold_look_max.value)]) >= 15 or max_values_per_row[(max_values_per_row <= threshold_look_min.value)] >= 15: #閾値範囲以外の個数が15個以上ある場合、注視判別.
            ...
            if len(max_indices_per_row[(max_indices_per_row >= 0) & (max_indices_per_row <= 50)]) >= 11: #10~50の範囲に11個以上ある場合  : 位相非反転
                gaze_flag.value = True 
            elif len(max_indices_per_row[(max_indices_per_row >= 51) & (max_indices_per_row <= 100)]) >= 11: #51~100の範囲に11個以上ある場合  : 位相反転
                gaze_flag2.value = True
        else:
            gaze_flag.value = False
            gaze_flag2.value = False
...
def main():
    ...
    shared_receive_list_3 = manager.list()  # 共有リスト超高Q10Hz
    shared_receive_list_4 = manager.list()  # 共有リスト超高Q15Hz

    flag_blink_1 = manager.Value('b', True)
    flag_blink_2 = manager.Value('b', True)

    chank_list_1 = manager.list() # 10Hz
    chank_list_2 = manager.list() # 15Hz
    chank_list_3 = manager.list() # 超高Q10Hz
    chank_list_4 = manager.list() # 超高Q15Hz

    receive_value_1 = manager.list()  # 共有リスト10Hz
    receive_value_2 = manager.list()  # 共有リスト15Hz
    receive_value_3 = manager.list()  # 共有リスト超高Q10Hz
    receive_value_4 = manager.list()  # 共有リスト超高Q15Hz

    clock_signal_1 = manager.Value('b', False)
    clock_signal_2 = manager.Value('b', False)

    adjust_chank_list_1 = manager.list() #10hz
    adjust_chank_list_2 = manager.list() #15hz
    adjust_chank_list_3 = manager.list() #超高Q10Hz
    adjust_chank_list_4 = manager.list() #超高Q15Hz
    ...
    # /************ 入力システム開始 ****************/
    process1 = multiprocessing.Process(target=func_serial, args=(priority1, com, shared_receive_list_1, receive_value_1, shared_receive_list_2, receive_value_2, shared_receive_list_3, receive_value_3, shared_receive_list_4, receive_value_4, clock_signal_1, clock_signal_2, lock))
    process2 = multiprocessing.Process(target=func_chank, args=(priority2, receive_value_1, receive_value_3, flag_blink_1, chank_list_1,chank_list_3, clock_signal_1, adjust_chank_list_1,adjust_chank_list_3, analysis_flag_1, 100, lock)) #10Hz: 1000data / 10Hz = 100
    process3 = multiprocessing.Process(target=func_chank, args=(priority3, receive_value_2, receive_value_4, flag_blink_2, chank_list_2,chank_list_4, clock_signal_2, adjust_chank_list_2,adjust_chank_list_4, analysis_flag_2, 167, lock)) #15Hz: 1000data / 6Hz = 166.666666 = 167         # fre_change_word.
    process4 = multiprocessing.Process(target=func_visual, args=(priority4, flag_blink_1, flag_blink_2, lock, chank_list_1, adjust_chank_list_1, chank_list_2, adjust_chank_list_2, chank_list_3, adjust_chank_list_3, chank_list_4, adjust_chank_list_4, gaze_flag_1, gaze_flag_1_2, gaze_flag_2, gaze_flag_2_2))
    process5 = multiprocessing.Process(target=func_analysis2, args=(priority5, adjust_chank_list_1 , adjust_chank_list_3 ,analysis_flag_1, gaze_flag_1, gaze_flag_1_2, gaze_flag_2, gaze_flag_2_2, adjust_chank_list_2, adjust_chank_list_4, analysis_flag_2, lock, threshold_look_10hz_max, threshold_look_6hz_max, threshold_non_look_10hz_max, threshold_non_look_6hz_max, threshold_look_10hz_min, threshold_look_6hz_min, threshold_non_look_10hz_min, threshold_non_look_6hz_min))
