        ...
        if ser.in_waiting > 0:  # 受信データがあるか確認
            ...
            if result:
                ...
                if measurement_command.value == 2: # 10Hz計測開始命令
                    ...
                    maxima_indices_topn, maxima_values_topn = find_local_maxima(np.array(store_list_non_look_10hz)[:, 0], top_n=200) #極大値top50を求める. 60秒-600個
                    minima_indices_topn, minima_values_topn = find_local_minima(np.array(store_list_non_look_10hz)[:, 0], top_n=200) #極小値top50を求める.
                    with lock:
                        threshold_non_look_10hz_max.value = np.mean(maxima_values_topn)
                        threshold_non_look_10hz_min.value = np.mean(minima_values_topn)

                    maxima_indices_topn, maxima_values_topn = find_local_maxima(np.array(store_list_non_look_6hz)[:, 0], top_n=150) #極大値top50を求める. 60秒-450個
                    minima_indices_topn, minima_values_topn = find_local_minima(np.array(store_list_non_look_6hz)[:, 0], top_n=150) #極小値top50を求める.
                    with lock:
