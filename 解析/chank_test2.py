...
def communicate_and_count(ser , received_list, lock):
...
def func_serial(com, shared_receive_list, lock):
    # global received_data  # グローバル変数を参照
    ser = serial.Serial(com, bitRate, timeout=None)
    communicate_and_count(ser, shared_receive_list, lock)
    # print("shared_receive_list: ", shared_receive_list)
    # print("len of shared_receive_list: ", len(shared_receive_list))



def func_chank(receive_list, lock):
    count_cunk = 4
    while True:
        with lock:  # ロックを使って排他制御
            if len(receive_list) > count_cunk * 1000:
                chanck_data = receive_list[(count_cunk - 3) * 1000 : count_cunk * 1000]
                # append_data_to_file(receive_data_txt, chanck_data)
                count_cunk += 1
                # print("chanck_data: ", chanck_data)
                # print("len of chanck_data: ", len(chanck_data))    
...
def main():
    ...
    # 並列処理で実行するプロセスを定義
    process1 = multiprocessing.Process(target=func_serial, args=(com, shared_receive_list, lock))
    process2 = multiprocessing.Process(target=func_chank, args=(shared_receive_list, lock))
