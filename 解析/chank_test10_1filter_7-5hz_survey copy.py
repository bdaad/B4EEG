...
def func_visual_preparation(priority, measurement_command, lock):
    ...
    blinking_image1_gyaku = BlinkingImage(position=(-1.0, 0.0), size=(0.5, 0.5), image_path="./circle.png", display_time=None, frequency=10, refresh_rate=refresh_rate, start_on=False, projection=projection)
    ...
    blinking_image3_gyaku = BlinkingImage(position=(0.5, 0.0), size=(0.5, 0.5), image_path="./circle.png", display_time=None, frequency=7.5, refresh_rate=refresh_rate, start_on=False, projection=projection)# fre_change_word.
    ...
    previous_time = time.time()
    while previous_time + 20/n > time.time(): # 60秒間ループ
        glClear(GL_COLOR_BUFFER_BIT) # カラーバッファをクリア
        for image in images: # 画像を描画
            if not image.update(): # 表示時間が経過したら
                images.remove(image)  # リストから削除する理由は、リストの要素を削除すると、リストの要素が前に詰められるため、forループが正しく動作するため.本当?
        ...
        if ser.in_waiting > 0:  # 受信データがあるか確認
            ...
            if result:
                ...
                if measurement_command.value == 2: # 10Hz計測開始命令
                    ...
                    print(np.array(store_list_look_10hz).shape)
                    save_2d_array_to_file(store_list_look_10hz, "non30s_look10hz20s_lookgyaku10hz20s_non30s")
