...
def communicate_and_count_test(ser , received_list_1, receive_value_1, received_list_2, receive_value_2, clock_signal_1, clock_signal_2, lock):
    ...
    # for i in range(10000000):
        ...
        if ser.in_waiting > 0:  # 受信データがあるか確認
            ...
            if result:
                # rec_all.append(result)　　# 何か3列で改行したり4列で改行したりよくワカラン
                # rec_all.extend(result)  # こっちじゃないらしい
    ...
    # 現在の日時を取得してファイル名に使用
    current_datetime = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    file_name = f"./save_data/all9s_{current_datetime}.txt"
...
def func_visual(priority, flag_blink_1, flag_blink_2, lock, chank_list_1, adjust_chank_list_1, chank_list_2, adjust_chank_list_2, gaze_flag_1, gaze_flag_1_2, gaze_flag_2, gaze_flag_2_2):
    ...
    blinking_image3 = BlinkingImage(position=(1.0, 0.0), size=(0.5, 0.5), image_path="./black.png", display_time=None, frequency=6, refresh_rate=refresh_rate, start_on=True, projection=projection)# fre_change_word.
    ...
    character_image1 = BlinkingImage(position=(-1.0, 0.2), size=(0.45, 0.45), image_path="./black.png", display_time=None, frequency=0, refresh_rate=refresh_rate, start_on=True, projection=projection)
    character_image1_on = BlinkingImage(position=(-1.0, 0.2), size=(0.45, 0.45), image_path="./black.png", display_time=None, frequency=0, refresh_rate=refresh_rate, start_on=True, projection=projection)
    character_image1_2_on = BlinkingImage(position=(-1.0, 0.2), size=(0.45, 0.45), image_path="./black.png", display_time=None, frequency=0, refresh_rate=refresh_rate, start_on=True, projection=projection)
    ...
    images = [blinking_image1, blinking_image3, character_image1, look_point_image]
...
# 文字の表示を管理 # fre_change_word.
    ...
    #         character_change_curennt_frame = blinking_image1.frame_count_not_reset
        ...
        if time.time() - end9 >= 9:
            print(time.time() - end9)
            # glfw.window_should_close(window) = 0  # これ許されるんか　許されなかった
            break

    glfw.destroy_window(window)
    glfw.terminate()

    # 241202 ウィンドウ閉じたら必ずデータ書き出すように変更
    with lock:
        save_2d_array_to_file(chank_list_1, "chank_list_1")
        save_2d_array_to_file(adjust_chank_list_1, "adjust_chank_list_1")
        save_2d_array_to_file(chank_list_2, "chank_list_2")
        save_2d_array_to_file(adjust_chank_list_2, "adjust_chank_list_2")
    sys.exit()  # destroy_windowより前だと不都合らしい
...
def func_serial(priority, com, shared_receive_list_1, receive_value_1, shared_receive_list_2, receive_value_2, clock_signal_1, clock_signal_2, lock):
    ...
    while True:
        try:
            ...
            # print('00000000000000000000')  # これは最初の一回だけ呼ばれるらしい
            break
