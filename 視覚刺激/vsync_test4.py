...
circle1 = BlinkingCircle(position=(-2.0, 0.0), size=0.2, color=(1.0, 1.0, 1.0), display_time=100, frequency=10, refresh_rate=refresh_rate, start_on=True)
...
while running:
    ...
    # フレーム間の経過時間を取得してフレームレートを計算
    elapsed_ms = clock.tick(refresh_rate)  # 経過時間（ミリ秒）
    if elapsed_ms > 0:
        frame_rate = 1000.0 / elapsed_ms
    else:
        frame_rate = float('inf')  # 経過時間が0の場合は無限大に設定
